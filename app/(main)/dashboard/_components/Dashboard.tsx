...
import { PlusCircle, Building2, ArrowRight, Waves } from "lucide-react";
...
const Dashboard = ({ startups, myStartups, myInvestedStartups }: { startups: Startup[], myStartups: Startup[], myInvestedStartups: Startup[] }) => {
    ...
    const handleOnExitSubmit = async () => {
        await axios.post(`${process.env.NEXT_PUBLIC_BASE_URL}/api/exitmailer`, {
            startupId: selectedStartupId,
        });

        toast({
            title: "Position Exit Initiated"
        });
    ...
    const handleOnExit = (startupId: string) => {
        setSelectedStartupId(startupId);
            ...
            <div className="container mx-auto px-4 py-8 space-y-8">
                ...
                <Tabs defaultValue="investments" className="space-y-6">
                    <TabsList className="w-full sm:w-auto">
                        <TabsTrigger value="allstartups" className="flex-1 sm:flex-none">
                    ...
                    <TabsContent value="allstartups" className="space-y-6">
                    ...
                    <TabsContent value="investments" className="space-y-6">
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                            {/* Example registered startups - replace with real data */}
                            {myInvestedStartups ? myInvestedStartups.map((startup: Startup) => (
                                <StartupCard
                                    key={startup.project_id}
                                    id={startup.project_id}
                                    name={startup.name}
                                    ...
                                    status="Active"
                                    isInvestor
                                    onExit={() => handleOnExit(startup.project_id)}
                                />
                            )) : (
                                <Card className="p-8 text-center">
                                    <div className="max-w-md mx-auto">
                                        ...
                                        <h3 className="text-lg font-semibold mb-2">No Startups Registered</h3>
                                        <p className="text-muted-foreground mb-6">
                                            You haven&apos;t invested in any startups yet. Please invest in your first one below.
                                        </p>
                                        <Button onClick={() => setActiveTab("allstartups")}>
                                            Invest in a Startup
                                            <ArrowRight className="ml-2 h-4 w-4" />
                                        </Button>
                                    </div>
                ...
                <Dialog open={isExitDialogOpen} onOpenChange={setIsExitDialogOpen}>
                    <DialogContent>
                        <DialogHeader>
                            ...
                            <DialogDescription>
                                You are about to exit your position from this startup. Are you sure you want to proceed?
                            </DialogDescription>
                        </DialogHeader>
                        <div className="space-y-4 py-4">
                            <Button
                                className="w-full"
                                onClick={handleOnExitSubmit}
                                variant="destructive"
                            >
                                Exit
                            </Button>
